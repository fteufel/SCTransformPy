...
def _parallel_wrapper(j):
    ...
    y = umi_bin[:,j].A.flatten() if isinstance(umi_bin, sp.sparse.spmatrix) else umi_bin[:,j].flatten()
...
def SCTransform(adata,min_cells=5,gmean_eps=1,n_genes=2000,n_cells=None,bin_size=500,bw_adjust=3,inplace=True):
    ...
    genes_cell_count = X.sum(0).A.flatten() if isinstance(X, sp.sparse.spmatrix) else X.sum(0).flatten()
    ...
    if n_cells is not None and n_cells < X.shape[0]:
        ...
        genes_cell_count_step1 = X[cells_step1].sum(0).A.flatten() if isinstance(X, sp.sparse.spmatrix) else  X[cells_step1].sum(0).flatten()
